...
function rangeToBounds(range: RangeKey) {
  const end = new Date();
  if (range === "ALL") return { start: null as string | null, end: ymd(end) };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (start) q = q.gte("occurred_on", start).lte("occurred_on", end);
  ...
  const kpis = useMemo(() => {
    ...
    const totalSpend = expenseOnly.reduce((s, r) => s + (Number(r.amount) || 0), 0);
    ...
    return { totalSpend, txCount, topCategory, avgPerDay };
  }, [rows, range]);
  ...
  const barData = useMemo(() => {
    ...
    const map = new Map<string, { label: string; spend: number; sortKey: number }>();
    for (const r of expenseOnly) {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        {[
          { label: "Spend (range)", value: idr.format(kpis.totalSpend) },
          ...
          >
            <Card className="border-emerald-700/40 bg-zinc-950/70 backdrop-blur text-zinc-100">
              <CardHeader className="pb-1 sm:pb-2 px-4 sm:px-5 pt-4 sm:pt-5">
                <CardTitle className="text-[13px] sm:text-sm text-zinc-400">{kpi.label}</CardTitle>
              </CardHeader>
              <CardContent className="px-4 sm:px-5 pb-4 sm:pb-5 text-xl sm:text-2xl font-semibold">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <motion.div className="lg:col-span-2" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="border-emerald-700/40 bg-zinc-950/70 backdrop-blur text-zinc-100">
            <CardHeader className="pb-1 sm:pb-2 px-4 sm:px-5 pt-4 sm:pt-5 flex items-center justify-between">
              <CardTitle className="text-[13px] sm:text-sm text-zinc-400">Spending</CardTitle>
              ...
              <div className="inline-flex rounded-lg border border-emerald-800/40 bg-zinc-900/60 p-1">
                {RANGE_OPTS.map((r) => (
                  <button
                    key={r}
                    onClick={() => setRange(r)}
                    className={`px-2 py-1 text-[11px] sm:text-xs rounded-md transition ${
                      range === r
            ...
            <CardContent className="h-56 sm:h-64 md:h-72 px-2 sm:px-4">
              {loading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barData} margin={{ top: 12, right: 12, left: 64, bottom: 8 }}>
                    ...
                    <Tooltip
                      ...
                      contentStyle={{ background: "#0b0f13", border: "1px solid #065f46", borderRadius: 8 }}
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
          <Card className="border-emerald-700/40 bg-zinc-950/70 backdrop-blur text-zinc-100">
            <CardHeader className="pb-1 sm:pb-2 px-4 sm:px-5 pt-4 sm:pt-5">
              <CardTitle className="text-[13px] sm:text-sm text-zinc-400">Categories — % of Spend</CardTitle>
            </CardHeader>
            <CardContent className="h-56 sm:h-64 md:h-72 px-2 sm:px-4">
              {loading ? (
                <div className="h-full grid place-items-center text-zinc-500">Loading…</div>
                  ...
                  <PieChart>
                    <Tooltip
                      ...
                      formatter={(value: any, _name: any, item: any) => {
                        ...
                        return [`${idr.format(v)} (${pct.toFixed(1)}%)`, item.payload.name];
                      }}
                    />
      ...
      {/* Recent — compact paddings */}
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <Card className="border-emerald-700/40 bg-zinc-950/70 backdrop-blur text-zinc-100">
          <CardHeader className="pb-1 sm:pb-2 px-4 sm:px-5 pt-4 sm:pt-5">
            <CardTitle className="text-[13px] sm:text-sm text-zinc-400">Recent Transactions</CardTitle>
          </CardHeader>
